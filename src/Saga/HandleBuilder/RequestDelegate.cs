            ...
            try
            {
                ...
                else
                {
                    ...
                    if (HandleType == HandleType.Handle)
                    {
                        ...
                        if (next != null)
                            await lifetimeScope.GetService<ISagaManager>().StartOrNext(next.Topic, localRollbackResult);
                    }
                    else
                    {
                        var prev = ConfigurationManager.GetConfig().GetPrevTopicByTopic(jsonData.FlowName, jsonData.Topic);
                        if (prev != null)
                            await lifetimeScope.GetService<ISagaManager>().StartOrNext(prev.RollbackTopic, localRollbackResult);
                    }
                }
            }
            catch (BaseSagaException e)
            {
                if (HandleType == HandleType.Handle)
                {
                    var prev = ConfigurationManager.GetConfig().GetPrevTopicByTopic(jsonData.FlowName, jsonData.Topic);
