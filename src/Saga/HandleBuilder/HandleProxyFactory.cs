...
namespace Saga.HandleBuilder
{
    ...
    public class HandleProxyFactory
    {
        ...
        public static void RegiterAllHandle(Func<ErrorModel, Task> errorHandle)
        {
            ...
            foreach (var type in Tools.GetAllTypeFromAttribute())
                foreach (var method in Tools.GetMethodFromAttribute(type))
                    delegateHandle.Add(CreateRequestDelegate(type, method.sagaAttr.Topic, method.sagaAttr.HandleType, method.methodInfo, errorHandle));
        }
        static Type DelegateType = typeof(RequestDelegate<,,>);
        static BaseRequestDelegate CreateRequestDelegate(Type serviceType, string Topic, HandleType handleType, MethodInfo method, Func<ErrorModel, Task> errorHandle)
        {
